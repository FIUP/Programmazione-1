...
int ** F(int *x)	//tipo ritorno: int**, passaggio di un puntatore per valore
{ (*x)++;			//deref, ottengo l'ogg puntato ("a" del main) e ne aumento l'R-val di 1
  ...
  return x; }		//ritorno il puntatore "x" per valore (cioè ne ritorno l'R-val), ma ha tipo int*
main()
...
Nota: se F avesse restituito "p" ci sarebbe stata concordanza con il tipo di ritorno (int**), ma sarebbe stato un dangling pointer,
in quanto p punta a x var locale della funzione; (per sicurezza verificare "Esempio 5" del .pdf "Lezione 9").
...
stampa 0, poi entrerà sempre nel ramo else ed il "continue" lo forzerà a saltare sempre "x++;" pertanto stamperà 10 volte 0 (fino a che l'indice non interromperà il ciclo).
...
int *posiz(int* X, int m, int el, int r, int c, int h, int lim1, int lim2, int lim3);
...
//pattern match sul sotto-array pieno con ordinamento per V-fette 
...
{ //PRE&POST: dato un "el" e dato un "m", "posiz" restituisce un punt all'elem "el+m"-esimo, che di certo esiste dato che gli elem sono h*l*lim1, e<=h*l*lim1-dimP ed m<dimP
  ...
  rig=r+((el+m)%h);			//indice della RIGa dove si trova l'elem d'interesse (rispetto a X)
  str=0+(((el+m)/h)%lim1);	//indice dello STRato dove si trova l'elem d'interesse (rispetto a X)
  col=c+((el+m)/(h*lim1));	//indice della COLonna dove si trova l'elem d'interesse (rispetto a X)
...
{	//PRE=(a partire dall'elem puntato da X seguono almeno dimP elem definiti).
  ...
  {
    if(*(posiz(X, m, el, r, c, H, lim1, lim2, lim3))!=*(P+m))
      trovato=false;
  }
  return trovato;
}	//POST=().
...
bool match(int* X, int r, int c, int H, int L, int lim1, int lim2, int lim3,int *P, int dimP, int el);
int *posiz(int* X, int elem, int r, int c, int H, int lim1, int lim2, int lim3);
...
int *posiz(int* X, int elem, int r, int c, int H, int lim1, int lim2, int lim3)
 { //PRE&POST: data la somma elem=el+m, "posiz" restituisce un punt all'elem "elem-esimo", che di certo esiste dato che gli elementi sono h*l*lim1, e<=h*l*lim1-dimP ed m<dimP
  ...
  rig= elem%H;			//indice della RIGa dove si trova l'elem d'interesse (all'interno del sotto-array di interesse)
  str= (elem/H)%lim1;	//indice dello STRato dove si trova l'elem d'interesse (all'interno del sotto-array di interesse)
  col= elem/(H*lim1);	//indice della COLonna dove si trova l'elem d'interesse (all'interno del sotto-array di interesse)
...
{	//PRE=(a partire dall'elem puntato da X seguono almeno dimP elem definiti).
  ...
  for(int m=0; m<dimP && trovato; m++)
  {
    if(*(posiz(X, el+m, r, c, H, lim1, lim2, lim3))!=*(P+m))
      trovato=false;
  }
