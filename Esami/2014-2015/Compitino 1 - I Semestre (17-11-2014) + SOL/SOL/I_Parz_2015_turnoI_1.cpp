...
*/
	...
	int nBestElem=0;				//numero di occorrenze di bestElem
	...
	while(t_rig<rp)					//gestisco (se ce ne sono) le righe piene
	{
		...
		while(t_col<10)
		{
			...
			/*
			La tripla (m,n,k) rappresenta il fatto che nella riga m il valore n ripete k volte. La tripla (m,n,k) è massima tra quelle esistenti
			...
			*/
			if(nElem>=nBestElem)							//p<k oppure p=k
			{
				...
				{
					if(elem>bestElem)						//q<n (se q=n dovrei vedere le righe, ma dato che le righe sono progressive so già che sarà maggiore)
					{
						nBestElem=nElem;
						bestElem=elem;
						riga=t_rig;
...
//doppio ciclo che stampa la matrice T con la giusta formattazione, utile per il debug
	...
	{
		t_col=0;					//reset indice
		while(t_col<10 && nBestElem<=10-t_col)	//se non restano sufficienti elementi da controllare per superare nBestElem posso fermarmi
		{
			int elem=T[t_rig][t_col]; 	//elemento di cui verificare in numero di occorrenze
			int nElem=0;				//numero di occorrenze di elem
			int k=t_col;				//indice ausiliario
			
			while(k<10 && nBestElem<=nElem+10-k)	//se prima o poi ci si accorge che le occorrenze di elem trovate più tutte le possibili future comunque non supererebbero nBestElem tanto vale fermarsi
			{
				if(elem==T[t_rig][k])
					nElem++;
				k++;
...
//	- PS: è evidente che anche se eur=0 verrà cominciato il while più esterno (a causa del "rp+1") ma poi nulla verrà fatto in quanto t_col<eur -> false.
	...
	t_col=0;									//reset indice
	while(t_col<eur && nBestElem<=eur-t_col)	//gestisco (se c'è) la riga incompleta
	{
			int elem=T[rp][t_col]; 				//elemento di cui verificare in numero di occorrenze
			int nElem=0;						//numero di occorrenze di elem
			int k=t_col;						//indice ausiliario
			
			while(k<eur && nBestElem<=nElem+eur-k)
			{
				if(elem==T[rp][k])
					nElem++;
				k++;
