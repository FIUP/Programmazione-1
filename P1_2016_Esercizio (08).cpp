...
main()	//PRE=(cin contiene dimT interi e dimP interi, seguite da 0<dimT<=100 e 0<dimP<=10).
{
	...
	while(t<dimT && (dimT-t>=dimP))		//R - cicla fino a quando ho elementi in T[0..dimT-1] && (dimP<=dimT-t<=dimT)
	{
		...
		if(stopP==false){
			nMatch++;
			t=t+dimP-1;   //Sposto indice t a t+dimP e sottraggo -1 perchè dopo c'è un t++
		}
		t++;
	}

	cout<<"n. match non sovrapposti = "<<nMatch<<endl;

}   //POST=(il programma stampa su cout il numero di match di P in T non sovrapposti)
