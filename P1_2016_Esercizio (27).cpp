...
void match(int *x, int*P, int dim, int dimP, int i, int count_match){		//conta i match
	if(i<dim-dimP+1){																		//se i è una posizione utile
		if(fin(x,i,P,dimP)){																//se c'è match 
			...
			cout<<"match n." << count_match << " a partire dalla posizione " << i<<endl;	//e stampiamo da dove inizia
		}
		match(x,P,dim,dimP,i+1,count_match);											//i match possono essere sovrapposti, dunque invochiamo di nuovo
	}																						//match a partire dalla prossima posizione
	else{ 																					//quando usciamo dalle posizioni valide
		cout<<"n. di match trovati="<<count_match<<endl;
		cout<<"end"<< endl;																//abbiamo finito
		return;
	}
}
...
int main(){
	...
	cin>> dimP;											//impostiamo la dimensione effettiva del pattern
	for(int i=0; i<dimP; i++)							//carichiamo gli elementi del pattern
		cin>> P[i];										//dal file di input
	
	
	match(X, P, n_el, dimP, 0, count_match);		//e invochiamo la funzione ricorsiva match partendo dall'indice 0
