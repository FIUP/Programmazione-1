...
FIFO pickric_infix(nodot* R, int& n, int k)
{
    ...
    n++;
    return concF(a, pickric_infix(R->right, n, k));
}
/*POST=(restituisce un valore FIFO f tale che f.primo sia una lista  tale che i nodi abbiano i 
campi info di valore uguale ai campi info dei nodi dell'albero Tree(R) che si trovano nelle 
seguenti posizioni rispetto all'ordine infisso:  
sia w=(k-vn)+1, allora le posizioni da considerare sono  w, w+k, w+2*k, w+3*k, eccetera . 
Inoltre n-1 è il numero di nodi presenti 
nell'albero che seguono, rispetto all'ordine infisso, il nodo dell'albero  in corrispondenza del quale  
si è creato il nodo f.ultimo)*/
